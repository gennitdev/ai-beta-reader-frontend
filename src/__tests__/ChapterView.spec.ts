...
vi.mock('@/composables/useChapterImages', async () => {
  ...
  return {
    useChapterImages: () => ({
      ...
      heroImageSrc: computed(() => null), activeImagePosition: computed(() => 0),
      albumImageCount: computed(() => 0), hasNextImage: computed(() => false),
      hasPrevImage: computed(() => false), illustrationToDeleteName: computed(() => ''),
  ...
  const wrapper = mount(ChapterView, {
    global: {
      stubs: {
        ...
        ConfirmDeleteModal: DeleteModalStub, ImageLightbox: { template: '<div><slot name="details" /></div>' },
      },
    },
  })
